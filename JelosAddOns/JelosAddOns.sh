...
if [ $? != 0 ]; then
  ESUDO=""
  export LD_LIBRARY_PATH="$LD_LIBRARY_PATH:/storage/roms/ports/JelosAddOns/libs"
  GREP="/storage/roms/ports/JelosAddOns/grep"
  WGET="/storage/roms/ports/JelosAddOns/wget"
  LANG=""
else
...
if [[ -e "/storage/.config/.OS_ARCH" ]] || [ "${OS_NAME}" == "JELOS" ]; then
  ...
  if [[ $isitthera == *"TheRA"* ]]; then
    if [ -d "/opt/tools/JelosAddOns/" ]; then
      toolsfolderloc="/opt/tools"
    else
      toolsfolderloc="/roms/ports"
    fi
  else
    if [ -d "/opt/system/Tools/JelosAddOns/" ]; then
...
cd $toolsfolderloc/JelosAddOns

$ESUDO $toolsfolderloc/JelosAddOns/oga_controls JelosAddOns.sh $param_device > /dev/null 2>&1 &

curversion="$(curl file://$toolsfolderloc/JelosAddOns/version)"

GW=`ip route | awk '/default/ { print $3 }'`
if [ -z "$GW" ]; then
  dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear \
...
if [ ! -d "/dev/shm/JelosAddOns" ]; then
  mkdir /dev/shm/JelosAddOns
fi
...
UpdateCheck() {
  ...
  if [[ "$gitversion" != "$curversion" ]]; then
	...
	dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear \
--yesno "\nThere's an update for JelosAddOns ($gitversion).  Would you like to download it now?" $height $width 2>&1 > /dev/tty0

    case $? in
	   0) 
		$WGET -t 3 -T 60 -q --show-progress "${website}JelosAddOns.zip" -O /dev/shm/JelosAddOns/JelosAddOns.zip 2>&1 | stdbuf -oL sed -E 's/\.\.+/---/g'| dialog \
			  --progressbox "Downloading and installing JelosAddOns update..." $height $width > /dev/tty0
		if [ ${PIPESTATUS[0]} -eq 0 ]; then
		  unzip -X -o /dev/shm/JelosAddOns/JelosAddOns.zip -d $toolsfolderloc/
		  mv -f $toolsfolderloc/JelosAddOns/JelosAddOns.sh $toolsfolderloc/.
		  if [ -f "$toolsfolderloc/JelosAddOns/tasksetter.sh" ]; then
		    rm -f "$toolsfolderloc/JelosAddOns/tasksetter.sh"
		  fi
		  if [ ! -z $isitext ]; then
			$ESUDO chmod -R 777 $toolsfolderloc/JelosAddOns
			$ESUDO chmod 777 $toolsfolderloc/JelosAddOns.sh
		  fi
		  dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear --msgbox "\n\nJelosAddOns updated successfully." $height $width 2>&1 > /dev/tty0
		  $ESUDO kill -9 $(pidof oga_controls)
		  $ESUDO rm -f /dev/shm/JelosAddOns/JelosAddOns.zip
		  $ESUDO systemctl restart oga_events &
		  exit 0
		else
		  dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear --msgbox "\n\nJelosAddOns failed to update." $height $width 2>&1 > /dev/tty0
		  $ESUDO rm -f /dev/shm/JelosAddOns/JelosAddOns.zip
		fi
        ;;
    esac
...
$WGET -t 3 -T 60 --no-check-certificate "$website"ports.md -O /dev/shm/JelosAddOns/ports.md
...
local unzipstatus
  ...
  msgtxt=$(cat /dev/shm/JelosAddOns/ports.md | $GREP "$1" | $GREP -oP '(?<=Desc=").*?(?=")')
  installloc=$(cat /dev/shm/JelosAddOns/ports.md | $GREP "$1" | $GREP -oP '(?<=locat=").*?(?=")')
  porter=$(cat /dev/shm/JelosAddOns/ports.md | $GREP "$1" | $GREP -oP '(?<=porter=").*?(?=")')
  ...
  case $? in
     0) $WGET -t 3 -T 60 -q --show-progress "$website$installloc" -O \
	    /dev/shm/JelosAddOns/$installloc 2>&1 | stdbuf -oL sed -E 's/\.\.+/---/g'| dialog --progressbox \
		"Downloading ${1} package..." $height $width > /dev/tty0
        unzip -o /dev/shm/JelosAddOns/$installloc -d /$whichsd/ports/ > /dev/tty0
        unzipstatus=$?
	    ...
	    $ESUDO rm -f /dev/shm/JelosAddOns/$installloc
	    ;;
	 *) 
	    ;;
  esac
...
userExit() {
  rm -f /dev/shm/JelosAddOns/ports.md
  $ESUDO kill -9 $(pidof oga_controls)
  $ESUDO systemctl restart oga_events &
  dialog --clear
  printf "\033c" > /dev/tty0
...
SetColorScheme() {
  if [ "$app_colorscheme" == "Default" ]; then
	export DIALOGRC=$toolsfolderloc/JelosAddOns/colorscheme/$app_colorscheme.dialogrc
    if [[ -e "$toolsfolderloc/JelosAddOns/JelosAddOns.sh" ]]; then
      sed -i "/export DIALOGRC\=\//c\export DIALOGRC\=\/" $toolsfolderloc/JelosAddOns/JelosAddOns.sh
    fi
    if [[ -e "$toolsfolderloc/JelosAddOns.sh" ]]; then
      sed -i "/export DIALOGRC\=\//c\export DIALOGRC\=\/" $toolsfolderloc/JelosAddOns.sh
    fi
  else
    export DIALOGRC=$toolsfolderloc/JelosAddOns/colorscheme/$app_colorscheme.dialogrc
    if [[ -e "$toolsfolderloc/JelosAddOns/JelosAddOns.sh" ]]; then
      sed -i "/export DIALOGRC\=\//c\export DIALOGRC\=$toolsfolderloc\/JelosAddOns\/colorscheme\/$app_colorscheme.dialogrc" $toolsfolderloc/JelosAddOns/JelosAddOns.sh
    fi
    if [[ -e "$toolsfolderloc/JelosAddOns.sh" ]]; then
      sed -i "/export DIALOGRC\=\//c\export DIALOGRC\=$toolsfolderloc\/JelosAddOns\/colorscheme\/$app_colorscheme.dialogrc" $toolsfolderloc/JelosAddOns.sh
    fi
  fi
}
...
ColorSchemeMenu() {
  ...
  dialog_config=(${toolsfolderloc}/JelosAddOns/colorscheme/*.dialogrc) # This creates an array of the full paths to all .dialogrc files
  dialog_config=("${dialog_config[@]##*/}") #Remove path prefix
  dialog_config=("${dialog_config[@]%.*}") #Get filename without extension
  cmd=(dialog \
    --clear \
	--backtitle "JelosAddOns v$curversion" \
	--title " [ Color Scheme Selection ] " \
	--no-collapse \
	--cancel-label "Back" \
	--menu "Select the JelosAddOns UI color scheme :" $height $width "15")
...
Settings() {
  if [[ ! -z $(cat $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt | $GREP x:b2) ]]; then
  ...
  while true; do
    ...
    for choice in $settingschoices; do
      case $choice in
        1) cp -f $toolsfolderloc/JelosAddOns/.Backup/donottouch.txt $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
		   if [ $? == 0 ]; then
		     dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear --msgbox "\n\nThe default gamecontrollerdb.txt has been successfully restored." $height $width 2>&1 > /dev/tty0
		   else
		     dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear --msgbox "\n\nThe default gamecontrollerdb.txt has failed to be restored.  Is the backup JelosAddOns subfolder or it's contents missing?" $height $width 2>&1 > /dev/tty0
		   fi
		   Settings
        ;;
		2) if [[ $curctrlcfg == "Switch to Xbox 360 Control Layout" ]]; then
		     sed -i 's/x:b2/x:b3/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			 sed -i 's/y:b3/y:b2/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			 if [[ $param_device != "anbernic" ]]; then
			   sed -i 's/a:b0/a:b1/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			   sed -i 's/b:b1/b:b0/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			 else
			   sed -i 's/a:b1/a:b0/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			   sed -i 's/b:b0/b:b1/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			 fi
		   else
		     sed -i 's/x:b3/x:b2/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			 sed -i 's/y:b2/y:b3/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			 if [[ $param_device != "anbernic" ]]; then
			   sed -i 's/a:b1/a:b0/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			   sed -i 's/b:b0/b:b1/g' $toolsfolderloc/JelosAddOns/gamecontrollerdb.txt
			 else
...
MainMenu() {
  local options=(
   $(cat /dev/shm/JelosAddOns/ports.md | $GREP -oP "(?<=Title=\").*?(?=\")|$power")
  )
...
MainMenuRTR() {
  local options=(
   $(cat /dev/shm/JelosAddOns/ports.md | $GREP 'runtype="rtr"' | $GREP -oP "(?<=Title=\").*?(?=\")|$power")
  )
