...
GREP="grep"
WGET="wget"
export DIALOGRC=/storage/roms/ports/JelosAddOns/colorscheme/jelosaddons.dialogrc
...
Emulators() {
  ...
  local settingsoptions=( 1 "Install Drastic Nintendo DS Emulator" 2 "Install Latest Duckstation Libretro PSX Core" 3 "Go Back" )
  ...
  while true; do
    ...
    for choice in $settingschoices; do
      case $choice in
        1) .drastic/install.sh
		   if [ $? == 0 ]; then
		     dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear --msgbox "\n\nDrastic Nintendo DS emulator has been installed." $height $width 2>&1 > /dev/tty0
		   else
		     dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear --msgbox "\n\nDrastic failed to install. " $height $width 2>&1 > /dev/tty0
		   fi
		   Settings
        ;;
		2) .duckstation/install.sh
		   if [ $? == 0 ]; then
		     dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear --msgbox "\n\nLatest Duckstation Libretro core has been installed." $height $width 2>&1 > /dev/tty0
		   else
		     dialog --clear --backtitle "JelosAddOns v$curversion" --title "$1" --clear --msgbox "\n\nDuckstation failed to install. " $height $width 2>&1 > /dev/tty0
		   fi
		   Settings
		;;
		3) TopLevel
...
TopLevel() {
  local topoptions=( 1 "All Available Ports" 2 "Ready to Run Ports" 3 "Optional Emulators" )
